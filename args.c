...
#ifndef lint
static	char sccsid[] = "@(#)args.c 1.1 94/10/31 SMI"; /* from S5R3.1 1.10 */
#endif
	...
	char	*argi[];
{
	...
	while (Rcheat(*argp++) != ENDARGS)
		argn++;
	/*
	 * sh_free old ones unless on for loop chain
	 */
	freedolh();
...
copyargs(from, n)
	char	*from[];
{
	register struct dolnod *np = (struct dolnod *)sh_alloc(sizeof(char**) * n + 3 * BYTESPERWORD);
...
/* After function invocation, sh_free positional parameters, 
