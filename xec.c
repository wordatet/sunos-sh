...
#ifndef lint
static	char sccsid[] = "@(#)xec.c 1.1 94/10/31 SMI"; /* from S5R3.1 1.28.1.1 */
#endif
	...
	if ((t = argt) && execbrk == 0)
	{
		...
		case TFORK:
			...
			else	/* this is the forked branch (child) of execute */
			{
				...
				if (type != TCOM)
				{
					execute(forkptr(t)->forktre, exec_link | 01, errorflg, (int *)0, (int *)0);
				}
		...
		case TUN:
			{
				...
				loopcnt++;
				while (execbrk == 0 && (execute(whptr(t)->whtre, 0, 0, (int *)0, (int *)0) == 0) == (type == TWH))
				{
