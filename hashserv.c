...
#ifndef lint
static	char sccsid[] = "@(#)hashserv.c 1.1 94/10/31 SMI"; /* from S5R3 1.10 */
#endif
...
{	
	...
	case S_IEXEC:
		if ((access(name, 1) == -1) || (stat(name, &statb) == -1))
			retval = (errno == EACCES ? 3 : 1);
		else {
			if (regflag && ((statb.st_mode & S_IFMT) != S_IFREG)){
					retval = 2;
					break;
			}
			if (!euid && !(statb.st_mode & (S_IEXEC|(S_IEXEC>>3)|(S_IEXEC>>6))))
				retval = 3;
			else
				retval = 0;
		} 
