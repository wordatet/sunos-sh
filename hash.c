...
#ifndef lint
static	char sccsid[] = "@(#)hash.c 1.1 94/10/31 SMI"; /* from S5R3 1.3 */
#endif
...
#define hash(str)		(int) (((unsigned long) (crunch(str) * (unsigned long)FACTOR)) >> shift)
...
hcreate()
{
	...
	table = (struct node **)sh_alloc(TABLENGTH * sizeof(struct node *));
	...
	shift = (bitsper * sizeof(long)) - LOG2LEN;
}
...
henter(item)
	ENTRY item;
{
	struct node	*p = (struct node *)sh_alloc(sizeof(struct node));
